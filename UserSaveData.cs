...
namespace HQMRanked
{
    public class UserSaveData
    {
        ...
        public static Dictionary<string, UserData> ReadUserData()
        {
            ...
            foreach(string s in raw)
            {
                ...
                if (user.Length < 3)
                    continue;
                UserData u = new UserData(user[0], user[1], new Moserware.Skills.Rating(double.Parse(user[2]), double.Parse(user[3])), int.Parse(user[4]), int.Parse(user[5]), int.Parse(user[6]), int.Parse(user[7]), user.Count() > 8 ? int.Parse(user[8]) : 9);
                users[u.Name] = u;
            }
            return users;
        }
        ...
        public static void SaveUserData()
        {
            ...
            {
                UserData d = u.Value;
                raw[i] = d.Name + "~" + d.Password + "~" + u.Value.Rating.Mean + "~" + u.Value.Rating.StandardDeviation + "~"+ d.GamesPlayed + "~"+d.Wins+"~"+d.Goals + "~"+ d.Assists + "~" + d.Division;
                i++;
            }
    ...
    public class UserData
    {
        ...
        public UserData(string name, string pw, Moserware.Skills.Rating r, int gamesPlayed = 0, int wins = 0, int goals = 0, int assists = 0, int division = 0)
        {
            Name = name;
            Password = pw;
            Rating = r;
